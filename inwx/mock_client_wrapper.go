...
func NewMockClientWrapper() *MockClientWrapper {
	return &MockClientWrapper{
		db:       make(map[string]*[]goinwx.NameserverRecord),
		idToZone: make(map[string]string),
	}
}

func (w *MockClientWrapper) login() (*goinwx.LoginResponse, error) {
	return &goinwx.LoginResponse{
		CustomerID: 1000,
		AccountID:  1000,
		TFA:        "",
...
func (w *MockClientWrapper) getRecords(domain string) (*[]goinwx.NameserverRecord, error) {
	recs, ok := w.db[domain]
	if !ok {
		return nil, fmt.Errorf("unable to retrieve records for zone %s: key not found in mock db", domain)
	}
	undeletedRecs := []goinwx.NameserverRecord{}
...
func (w *MockClientWrapper) createRecord(r *goinwx.NameserverRecordRequest) error {
	recs, ok := w.db[r.Domain]
	if !ok {
		return fmt.Errorf("zone %s not found", r.Domain)
	}
	id := strconv.Itoa(len(*recs))
	newRecs := append(*recs, goinwx.NameserverRecord{
		ID:       id,
		Name:     r.Name,
		Type:     r.Type,
		Content:  r.Content,
...
func (w *MockClientWrapper) findRecord(recs *[]goinwx.NameserverRecord, recID string) int {
...
func (w *MockClientWrapper) CreateZone(zone string) {
	...
	w.db[zone] = &[]goinwx.NameserverRecord{}
}
